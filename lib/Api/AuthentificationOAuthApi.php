...
class AuthentificationOAuthApi
{
     ...
     */
    public function apiAuthAccessTokenPostRequest($client_id, $grant_type, $code)
    {
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function apiAuthAuthorizeGetRequest($client_id, $redirect_uri, $response_type)
    {
        // verify the required parameter 'client_id' is set
        if ($client_id === null || (is_array($client_id) && count($client_id) === 0)) {
            throw new \InvalidArgumentException(
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function apiAuthPostRequest($auth_access_token)
    {
        ...
        if (isset($auth_access_token)) {
            if ($headers['Content-Type'] === 'application/json') {
                $httpBody = \GuzzleHttp\json_encode(ObjectSerializer::sanitizeForSerialization($auth_access_token));
            } else {
                $httpBody = $auth_access_token;
            }
